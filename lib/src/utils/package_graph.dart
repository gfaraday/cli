...
class PackageGraph {
  ...
  factory PackageGraph.forPath(String packagePath) {
    ...
    var pubspec = File(path.join(packagePath, 'pubspec.yaml'));
    ...
    var packagesFile = File(path.join(packagePath, '.packages'));
    ...
    packagesFile.readAsLinesSync().skip(1).forEach((line) {
      ...
      if (!uri.isAbsolute) {
        uri = Uri.file(path.join(packagePath, uri.path));
      }
...
PackageDependencyType _dependencyType(Object? source) {
  ...
  switch (typeString) {
    ...
    case 'sdk':
      return PackageDependencyType.sdk;
    default:
